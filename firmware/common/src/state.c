...
const uint16_t max_time[] = {36000, 54000, 54000, 33000, 1200};
const uint16_t cooldown_time = 4800; // Cooldown of 8 minutes (in seconds)
...
void rt_send_tx_package(frame_type_t type) {
  ...
  switch (type) {
    case FRAME_TYPE_PERIODIC:
      ...
      // wheel max speed
      if (rt_vars.ui8_street_mode_enabled && rt_vars.ui8_street_mode_enabled)
      {
...
void rt_send_tx_package_tsdz8(frame_type_t type) {
	uint8_t *ui8_usart1_tx_buffer = uart_get_tx_buffer();
...
void communications(void) {
  ...
  if (g_motor_init_state == MOTOR_INIT_READY)
    if(uart_get_motor_type() == MOTOR_TSDZ8) {
      rt_send_tx_package_tsdz8(FRAME_TYPE_PERIODIC);
    }
    else{
...
/*void cooling_down(void) {
    ...
    // If reach 4 hour runtime limit, enter the test mode, only assist levels 0 and 1 are allowed
    if (total_runtime >= runtime_limit){
      // Ensure that after this point, only assist levels 0 and 1 are allowed
      if (ui_vars.ui8_assist_level > 1) {
          ui_vars.ui8_assist_level = 1;
      }
    }else{
      ...
      if (ui_vars.ui8_assist_level || is_cooling_down) {
          ...
          if (ui_vars.ui8_assist_level) {  // Ensure there's a valid assist level
              if (is_cooling_down) {  // If in cooling state
                  ...
                  // unless the assist level is 1 (no counting for level 1)
                  if (ui_vars.ui8_assist_level != 1) {
